...
happyExpList = Happy_Data_Array.listArray (0,42) ([1408,704,128,1536,0,0,22,11,32768,24577,45057,32768,0,5632,2816,0,0,0,0,0,0
	])

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_parseExpr","Expr","Factor","Atom","Var","NUMBER","MINUS","PLUS","LEFTP","RIGHTP","DIV","MUL","%eof"]
        bit_start = st * 15
        bit_end = (st + 1) * 15
...
happyReduction_10 (HappyTerminal (Number happy_var_1))
	 =  HappyAbsSyn7
		 (Var happy_var_1
	)
...
happyNewToken action sts stk (tk:tks) =
	...
	case tk of {
	Number happy_dollar_dollar -> cont 8;
	MinusT -> cont 9;
	PlusT -> cont 10;
	LeftP -> cont 11;
	RightP -> cont 12;
